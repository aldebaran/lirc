...
#ifdef PIPE_COM
      const char msg_nodata[MAX_BUF_SIZE] = "#MANIP# No data for 10 secs, operation abort"
        "\nThe last button did not seem to generate any signal.\nClick on NEXT to continue.\n\0";
      write(wrfd, msg_nodata, strlen(msg_nodata));
      do{
        numread = read(rdfd, rdbuf, MAX_BUF_SIZE);
        rdbuf[numread] = '\0';
        if(!strcmp(rdbuf, "#ESCAPE#")) return -1;
      }
      while(strcmp(rdbuf, "#NEXT#"));
#else
      getchar();
...
#ifdef PIPE_COM
      const char msg_key[MAX_BUF_SIZE] = "#KEY# Please enter the name for the next button (Click on \"Finish\" to finish recording)\0";
      write(wrfd, msg_key, strlen(msg_key));
      numread = read(rdfd, rdbuf, MAX_BUF_SIZE); 
      rdbuf[numread+1] = '\0';
      if(!strcmp(rdbuf, "#ESCAPE#")) return -1;
      printf("\n VAL de rdbuf = %s\n", rdbuf);
...
#ifdef PIPE_COM
  const char msg_toggle[MAX_BUF_SIZE] =  "#MANIP# Please press an arbitrary button repeatedly as fast as possible. "
                                "Make sure you keep pressing the SAME button and that you DON'T HOLD "
                                "the button down!. "
                                "Click on NEXT to continue.\0";
  write(wrfd, msg_toggle, strlen(msg_toggle));
...
#ifdef PIPE_COM
  const char msg_hold1s[MAX_BUF_SIZE] =  "#MANIP#Now start pressing buttons on your remote control. "
    "It is very important that you press many different buttons and hold them "
    "down for approximately one second. "
    "Click on NEXT now to start recording.\0";

  write(wrfd, msg_hold1s, strlen(msg_hold1s));
